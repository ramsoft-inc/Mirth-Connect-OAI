...
public class DataPruner implements Runnable {
    ...
    public DataPruner() {
        this.retryCount = 3;
        this.skipIncomplete = false;
        this.skipStatuses = new Status[] { Status.PENDING };
        ...
        for (Channel channel : channels) {
            if (!(channel instanceof InvalidChannel)) {
                ...
                switch (properties.getMessageStorageMode()) {
                    ...
                    case PRODUCTION:
                    case RAW:
                    /* Also perform Data Pruner when Message Storage is disabled */
                    case DISABLED:
    ...
    public PruneResult pruneChannel(String channelId, String channelName, Calendar messageDateThreshold, Calendar contentDateThreshold, String archiveFolder, boolean channelArchiveEnabled) throws InterruptedException, DataPrunerException {
        ...
        while (true) {
            ...
            try {
                ...
                /* Perform delete D_MS, D_MA tables when disable Message Storage */
                if (messageIds.hasNext() == false)
                {
                    Map<String, Object> paramCustom = new HashMap<String, Object>();
                    paramCustom.put("localChannelId", localChannelId);
                    runDeleteQueries(paramCustom, false, result);
                }
                return result;
            } catch (InterruptedException e) {
                throw e;
    ...
    private void runDeleteQueries(Map<String, Object> params, boolean contentOnly, PruneResult result) {
        if (contentOnly) {
            ...
            /* Perform to delete D_MA tables */
            if (DatabaseUtil.statementExists("Message.pruneCustomAttachments")) {
                runDelete("Message.pruneCustomAttachments", params);
            }
            if (DatabaseUtil.statementExists("Message.pruneCustomMetaData")) {
                runDelete("Message.pruneCustomMetaData", params);
            }
