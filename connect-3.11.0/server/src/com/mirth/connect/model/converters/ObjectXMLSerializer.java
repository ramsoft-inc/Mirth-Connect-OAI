...
public class ObjectXMLSerializer extends XStreamSerializer {
    ...
    @SuppressWarnings("unchecked")
    public <T> T deserializeChannelGroup(String serializedObject, Class<T> expectedClass) {
        DonkeyElement element = null;
        String preUnmarshalXml = null;
        ...
        try {
            if (skipMigration(expectedClass)) {
                ...
                if (expectedClass == ChannelGroup.class) {
                    DonkeyElement listChannelElement = element.getChildElement("channels");
                    DonkeyElement finalListChannelElement = new DonkeyElement("<channels/>");
                    // Iteration each channel in the group for migration
                    for (DonkeyElement child : listChannelElement.getChildElements()) { 
                        preUnmarshalXml = null;
                        try {                            
                            preUnmarshalXml = child.toXml();                                
                            child = ImportConverter3_0_0.migrate(child, Channel.class);
                            finalListChannelElement.addChildElementFromXml(child.toXml());
                        } catch (DonkeyElementException e) {
                        }
                    }
                    element.removeChild("channels");
                    element.addChildElementFromXml(finalListChannelElement.toXml());
                }                
